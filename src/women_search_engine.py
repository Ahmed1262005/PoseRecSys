...
class WomenSearchEngine:
    ...
    def search_with_filters(
        ...
        text_embedding = query_embedding if query_embedding is not None else self.encode_text(query)
        text_embedding = text_embedding.astype('float32').tolist()
        vector_str = f"[{','.join(map(str, text_embedding))}]"
    ...
    def search_multimodal(
        ...
        Args:
            ...
            exclude_product_ids: Product IDs to exclude.
            query_embedding: Pre-computed embedding (skips encode_text if provided).

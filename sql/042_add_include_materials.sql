-- =====================================================
-- Add include_materials parameter to exploration keyset functions
--
-- Problem: We can exclude materials at SQL level (exclude_materials)
-- but cannot INCLUDE (positive filter) materials at SQL level.
-- When user selects "cotton,linen" as include_materials, we must
-- filter in Python after fetching, which is wasteful.
--
-- Solution: Add include_materials text[] parameter to both
-- get_exploration_keyset and get_exploration_keyset_with_brands.
-- SQL uses array overlap (&&) to check if product materials
-- contain at least one of the requested materials.
--
-- Logic: AND (include_materials IS NULL OR (p.materials && include_materials))
-- =====================================================

-- =============================================================
-- get_exploration_keyset: Add include_materials parameter
-- =============================================================

DROP FUNCTION IF EXISTS get_exploration_keyset(text, text[], text[], text[], text[], numeric, numeric, text, float, uuid, int, text[], text[], text[], float, float, text[], text[], float, boolean, boolean, int, uuid[]);
...
CREATE OR REPLACE FUNCTION get_exploration_keyset(
    ...
    -- Seen history exclusion
    exclude_product_ids uuid[] DEFAULT NULL,
    -- Material inclusion filter (NEW)
...
BEGIN
    ...
    WHERE p.in_stock = true
      ...
      -- Material inclusion filter (NEW) - product must contain at least one requested material
...
-- =============================================================
-- get_exploration_keyset_with_brands: Add include_materials parameter
-- =============================================================

DROP FUNCTION IF EXISTS get_exploration_keyset_with_brands(text, text[], text[], text[], text[], numeric, numeric, text, float, uuid, int, text[], text[], text[], float, float, text[], text[], float, text[], boolean, boolean, int, uuid[]);
...
BEGIN
    ...
    SELECT
        ...
        p.hero_image_url,
        p.gallery_images,
        -- Score calculation with brand boost
        CASE
            WHEN new_arrivals_only THEN
                CASE
...
SELECT 'Added include_materials parameter to exploration keyset functions for SQL-level material inclusion filtering' as status;

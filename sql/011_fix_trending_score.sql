-- Fix: trending_score is 0, not NULL, so COALESCE doesn't work as expected
-- Use NULLIF to convert 0 to NULL first

-- Drop and recreate with fixed score handling
...
-- Test: Should return 0 rows now (all items have score 0.5, cursor is 0.5 with min UUID)
...
SELECT product_id, similarity FROM get_trending_keyset(
    filter_gender := 'female',
    p_limit := 3
);

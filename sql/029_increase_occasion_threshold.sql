-- =====================================================
-- Increase Occasion Threshold Default from 0.20 to 0.30
-- Run after 028_add_computed_attributes.sql
--
-- This migration updates the default occasion threshold
-- to 0.30 for more selective filtering of office/casual/evening occasions.
-- With 0.20, ~52% of products passed the filter (too permissive).
-- With 0.30, filtering is more precise.
-- =====================================================

-- Update match_products_keyset
DROP FUNCTION IF EXISTS match_products_keyset(vector(512), text, text[], text[], text[], text[], numeric, numeric, float, uuid, int, text[], text[], text[], float, float, text[], text[], float);
...
CREATE OR REPLACE FUNCTION match_products_keyset(
    ...
    occasion_threshold float DEFAULT 0.30,  -- INCREASED from 0.20
    -- Pattern filters
...
-- Update get_trending_keyset
DROP FUNCTION IF EXISTS get_trending_keyset(text, text[], text[], text[], text[], numeric, numeric, float, uuid, int, text[], text[], text[], float, float, text[], text[], float);
...
-- Verification
SELECT 'Occasion threshold updated to 0.30' as status;

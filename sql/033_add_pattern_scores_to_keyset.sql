-- =====================================================
-- Add computed_pattern_scores to keyset functions
-- Run after 032_lower_occasion_threshold.sql
-- =====================================================
...
RETURNS TABLE(
    ...
    computed_pattern_scores jsonb  -- NEW
)
LANGUAGE plpgsql AS $$
DECLARE
    effective_seed text;
BEGIN
    ...
    SELECT
        ...
        COALESCE(p.computed_pattern_scores, '{}'::jsonb) as computed_pattern_scores  -- NEW
    FROM products p
...
SELECT 'Pattern scores added to keyset functions' as status;
